...
> **A note on this project.** wsx approaches git worktree based agent development from a hands on perspective, choosing to delegate to other tools rather than be an all in one IDE.  Many of wsx's features are only accessible from the CLI and empower the user to define their own behaviors. If you want the power of multi agent worktree based development with an orchestration CLI but like working in the terminal, want to use your own tools like Neovim, Emacs and Lazygit, then this might just be something you’d enjoy using too.  Feedback, ideas, and contributions are welcome.
