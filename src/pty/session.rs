...
// `read_hermes_spawn_marker` (idempotency guard) and `write_hermes_spawn_marker`
// are called by `prepare_hermes_workspace` below.
pub(crate) use crate::pty::session_detect::{read_hermes_spawn_marker, write_hermes_spawn_marker};
// The remaining marker/sqlite internals are exercised only by this file's
// test module (via `super::`); re-export them under cfg(test) so production
// builds don't see an unused import.
#[cfg(test)]
pub(crate) use crate::pty::session_detect::{
    cache_hermes_session_id_in_marker, latest_hermes_session_id,
};

// Per-agent command construction now lives in `command`. Re-export the
// builders (called by `spawn_session` below) and `compose_injected_prompt`
// (called by `prepare_hermes_workspace`); the rename-prompt / shell-quote
// helpers are reached only by this file's test module, so gate those re-exports
// under cfg(test).
pub(crate) use crate::pty::command::compose_injected_prompt;
...
pub(crate) use crate::pty::command::{
    render_rename_system_prompt, render_rename_system_prompt_hermes,
