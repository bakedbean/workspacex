...
3. Patch `~/.config/waybar/config.jsonc` after making a timestamped `.bak`:
   ...
   array) and insert `"custom/wsx"` into `modules-left` (falling back to
   `modules-right`), immediately after `"custom/omarchy"` when that entry
   exists, else appended to the array. The patch is targeted text editing of
