...
# contract that bootstrap.sh established, deriving the four XDG/agent dirs from
...
# Isolated user config (wsx reads $XDG_CONFIG_HOME/wsx/theme.toml), so a sandboxed
# TUI never picks up — and `wsx theme init` never writes — the real ~/.config/wsx.
